import React from "react";
import ReactDOM from "react-dom/client";
import "./index.css";
const pizzaData = [
  {
    name: "Focaccia",
    ingredients:
      "Bread with italian olive oil and rosemary",
    price: 6,
    photoName: "pizzas/focaccia.jpg",
    soldOut: false,
  },
  {
    name: "Pizza Margherita",
    ingredients: "Tomato and mozarella",
    price: 10,
    photoName: "pizzas/margherita.jpg",
    soldOut: false,
  },
  {
    name: "Pizza Spinaci",
    ingredients:
      "Tomato, mozarella, spinach, and ricotta cheese",
    price: 12,
    photoName: "pizzas/spinaci.jpg",
    soldOut: false,
  },
  {
    name: "Pizza Funghi",
    ingredients:
      "Tomato, mozarella, mushrooms, and onion",
    price: 12,
    photoName: "pizzas/funghi.jpg",
    soldOut: false,
  },
  {
    name: "Pizza Salamino",
    ingredients:
      "Tomato, mozarella, and pepperoni",
    price: 15,
    photoName: "pizzas/salamino.jpg",
    soldOut: true,
  },
  {
    name: "Pizza Prosciutto",
    ingredients:
      "Tomato, mozarella, ham, aragula, and burrata cheese",
    price: 18,
    photoName: "pizzas/prosciutto.jpg",
    soldOut: false,
  },
];

function App() {
  return (
    <div className="container">
      <Header />
      <Menu />
      <Footer />
    </div>
  );
}
function Header() {
  return (
    <header className="header">
      <h1>React Pizza Co</h1>
    </header>
  );
}
function Menu() {
  const pizzaArr = pizzaData;
  const pizzaLen = pizzaArr.length;
  return (
    <main className="menu">
      <h2>Our Menu</h2>
      <ul className="pizzas">
        {pizzaLen > 0 &&
          pizzaArr.map((pizza) => (
            <Pizza
              pizzaObj={pizza}
              key={pizza.name}
            />
          ))}
      </ul>
    </main>
  );
}
function Pizza(props) {
  let soldOut = props.pizzaObj.soldOut;
  soldOut = soldOut ? "sold-out" : "";
  return (
    <div className={`pizza ${soldOut}`}>
      <img
        src={props.pizzaObj.photoName}
        alt={props.pizzaObj.name}
      />
      <div>
        <h3>{props.pizzaObj.name}</h3>
        <p>{props.pizzaObj.ingredients}</p>
        <span>
          {props.pizzaObj.soldOut
            ? "sold-out"
            : props.pizzaObj.price}
        </span>
      </div>
    </div>
  );
}
function Footer() {
  const hour = new Date().getHours();
  const open = 20;
  const close = 24;
  const isOpen = hour >= open && hour <= close;
  return (
    <footer className="footer">
      {isOpen ? (
        <Order />
      ) : (
        <p>
          We are closed right now come between{" "}
          {open}:00 and {close}:00. Time right now
          is {new Date().toLocaleTimeString()}
        </p>
      )}
    </footer>
  );
}
function Order() {
  return (
    <div className="order">
      <p>
        {new Date().toLocaleTimeString()}. We are
        open right now. Visit us or oder onlie
        now.
      </p>
      <button className="btn">Order Now</button>
    </div>
  );
}

const root = ReactDOM.createRoot(
  document.getElementById("root")
);

root.render(<App />);
